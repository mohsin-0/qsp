    ...
    for bond in bonds:
        ...
        if orientation == "H":
            kernel = sp.linalg.null_space(
                ncon([Q1, Q2], [(-1, -2, 1, -5, -7), (1, -3, -4, -6, -8)]).reshape(
                    -1, 16**2
                )
            )
        ...
        if orientation == "V":
            kernel = sp.linalg.null_space(
                ncon([Q1, Q2], [(-1, -3, -4, 1, -7), (-2, 1, -5, -6, -8)]).reshape(
                    -1, 16**2
                )
            )
...
def apply_evolution_opeerator(parent_hamiltonian, peps, tau, Tmax):
    ...
    for bond, curr_term in parent_hamiltonian.terms.items():
        ...
        curr_gate = sp.linalg.expm(
            curr_term.reshape(d**2, d**2) * -1j * (tau / 1) * Tmax
        ).reshape((d, d, d, d))
...
if __name__ == "__main__":
    ...
    for t_it, t in enumerate(ts):
        ...
        _, parent_hamiltonian = construct_parent_hamiltonian(
            tensor_grid, bonds, num2pt, Lx=Lx, Ly=Ly
        )
        ...
        exp_val = np.real(
            peps.compute_local_expectation(parent_hamiltonian.terms, max_bond=32)
        )
        # denom = np.sqrt((peps.H@peps)*(aklt_peps.H@aklt_peps))
        target_fidelity = np.abs(peps.H @ aklt_peps)  # /denom)

        print(
            f"{t_it=:03d}, {t=:.2f}, {s=:.5f}, {exp_val=:.08f}, {target_fidelity=:.08f}"
        )
