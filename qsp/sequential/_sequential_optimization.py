    ...
    for it in reversed(range(len(unitary_layers))):
        ...
        for start_indx, end_indx, Gs, _, _ in Gs_lst:
            for it in range(start_indx, end_indx+1):
                if (it==end_indx):
                    params_count = apply_quimb_unitary(Gs[it-start_indx].data, quimb_circ, it, it+1, gid_to_qubit)
...
def sequential_unitary_circuit_optimization(target_mps, unitaries, n_iter, nhop, verbose=False): 
    """build parametrized circuit from sequential unitary ansatz
    """        
    quimb_circ, gid_to_qubit, ttl_params_count = generate_circ_from_unitary_layers(unitaries, target_mps.L)
    ...
    # if verbose:
    print(f'number of variational params in the circuit (from sequential algorithm) are '
          f'{ttl_params_count}') 
