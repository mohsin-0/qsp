...
def mps_to_isometry_list(tens_list, canonical_form): 
    ...
    if canonical_form=='left':
        isometry_list = [reshape_lft2inds(A) 
                         if tid!=0  else A 
                         for tid, A in enumerate(tens_list)]
        
    if canonical_form=='right':
        isometry_list = [reshape_rht2inds(A) 
                         if tid!=(L-1) else A 
                         for tid, A in enumerate(tens_list)]
...
def canonical_form_sanity_check(tens_list, canonical_form):
    ...
    for u in isometry_list:
        ...
        if len(shape)>1: 
            if shape[0]>shape[1]:
                chks.append(np.allclose(tf.eye(shape[1], dtype=tf.complex128), 
                                        tf.linalg.adjoint(u)@u))
                
            else:
                chks.append(np.allclose(tf.eye(shape[0], dtype=tf.complex128), 
...
def quimb_mps_to_tf_mps(mps, canonical_form):
    ...
    isometry_list = [isometry.reshape(-1,1) 
                     if len(isometry.shape)==1 else isometry  
