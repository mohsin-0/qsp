...
import tsp_lcu_optimization_misc as tsp_lcu_opt_misc
...
def compute_overlap(lcu_list_var, lcu_shapes_list, 
    ...
    for it in range(no_of_layers):
        ...
        temp = tsp_lcu_opt_misc.mps_overlap(curr_mps, 
                                            curr_mps_shapes,
    ...
    for it1 in range(no_of_layers):
        ...
        for it2 in range(no_of_layers):
            ...
            temp = tsp_lcu_opt_misc.mps_overlap(curr_mps1, 
                                                curr_mps_shapes1,
                                                curr_mps2, 
                                                curr_mps_shapes2)
...
def x_to_tf_lcu_list(x, shape_structure):
    ...
    for shape in shape_structure:
        ten = tf.Variable(np.array(x[offset:(offset+np.prod(shape))], 
...
# @tf.function
def value_and_gradient(lcu_isometry_list_var, 
                       lcu_shapes_list, 
                       target_isometry_list, 
                       target_shapes_list, 
                       kappas, 
    ...
    with tf.GradientTape() as tape:
        overlap = compute_overlap(lcu_isometry_list_var, 
                                  lcu_shapes_list, 
                                  target_isometry_list, 
                                  target_shapes_list, 
                                  kappas, 
...
def lcu_unitary_circuit_optimization(target_mps, 
                                     kappas, 
                                     lcu_mps, 
                                     max_iterations,
                                     verbose=False): 
    ...
    target_isometry_list, target_shapes_list = tsp_lcu_opt_misc.quimb_mps_to_tf_mps(target_mps, canonical_form='left')
    ...
    lcu_shapes_list = []
    for mps in lcu_mps:
        isometry_list, shapes_list = tsp_lcu_opt_misc.quimb_mps_to_tf_mps(mps, canonical_form='left')
